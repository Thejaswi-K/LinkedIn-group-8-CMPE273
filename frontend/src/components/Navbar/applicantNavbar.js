...
class applicantNavbar extends Component {
    ...
    handleLogout = () => {
        let loggedInUser = capitalizeFirstLetter(extractNameFromEmail(jwtDecode(localStorage.getItem('applicantToken')).email));
        ...
        if(!this.state.isRecruiter && this.state.isLogged){
            ...
            userLogin = (
                <div id="login" className="dropdown" tabindex="-1" role="presentation">
                    <button style= {{fontSize: "1.6rem", color: "#fff", border: "none", paddingBottom: "18px", fontWeight: "600"}} aria-haspopup="true" aria-expanded="false" className="site-header-nav__toggle Dropdown__toggle " id="dropdownMenuButton"
                        label="Login" data-toggle="dropdown">
                ...
                <div>
                    ...
                    <nav className="navbar navbar-expand-lg navbar-light bg-light">
                        <div className="navbar-header row" style ={{marginLeft:10}}>
                            ...
                            {search}
                            <input style ={{fontSize: '100%', weight: 300, marginLeft: 25, marginTop: 8, marginBottom: 5, color: '#fff'}} tabindex="1" id="login-submit" className="header wrapper login-form login submit-button" type="submit"
